...
def changeStateToString(state): #in order to optimize visibility of Qtable, we change state to string
    stateString_str = ''
    for item in state:
        if item is True:
            stateString_str += '1'
...
class Q_Table: #define Qtable
    def __init__(self, action_space):
        self.actions = action_space
        self.q_table = pd.DataFrame(columns=action_space, dtype=np.float64)
        self.epsilon = initialEpsilon

    def checkStateExist(self, state): #Add a new state if it was not in the q table
    ...
    def getMaxAction(self, stateString, feasible_actions):# get the action which has the max risk
        ...
        while (stepCounterEpisode <= N_STEPS_EPISODE) and not reset:
            ...
            if isRunning:
                ...
                _, risk = client.simxCallScriptFunction("getMaxRisk@RiskMetricCalculator", "sim.scripttype_childscript",
                                                                1, client.simxServiceCall())
                client.simxCallScriptFunction("resetMaxRisk@RiskMetricCalculator", "sim.scripttype_childscript", 1,
                                                      client.simxServiceCall())
                risksFromThisEpisode.append(risk)

                #define reward of Qlearning
                reward = float(risk)

                # retrieve next state
                nextState = None
                while nextState == None:
                        # try to get next state and retry if it fails (sometimes multiple timesteps are needed until simulator returns valid result)
                    try:
                ...
                Qnow = agent_q.q_table.loc[stateString, action]
                ...
                agent_q.q_table.loc[stateString, action] += lr * (Qtarget - Qnow)
